...
class PyraxInitTest(unittest.TestCase):
    ...
    def setUp(self):
        super(PyraxInitTest, self).setUp()
    ...
    def tearDown(self):
        super(PyraxInitTest, self).tearDown()
    ...
    def test_set_credentials(self):
        ...
        self.assertEqual(self.identity.username, self.username)
        self.assertEqual(self.identity.password, self.password)
        self.assert_(self.identity.authenticated)

    def test_set_bad_credentials(self):
        self.assertRaises(exc.AuthenticationFailed, pyrax.set_credentials,
                "bad", "creds", identity=self.identity)
    ...
    def test_set_credential_file(self):
        with utils.SelfDeletingTempfile() as tmpname:
            with open(tmpname, "wb") as tmp:
                ...
                tmp.write("api_key = %s\n" % self.password)
            pyrax.set_credential_file(tmpname, identity=self.identity)
            self.assertEqual(self.identity.username, self.username)
            self.assertEqual(self.identity.password, self.password)
            self.assert_(self.identity.authenticated)
    ...
    def test_keyring_auth(self):
        ...
        pyrax.keyring_auth(identity=self.identity)
    ...
    def test_auth_with_token(self):
        ...
        pyrax.auth_with_token(tok, identity=self.identity, tenant_name=tname)
        self.assertTrue(self.identity.authenticated)
        self.assertEqual(self.identity.token, tok)
        self.assertEqual(self.identity.tenant_name, tname)
    ...
    def test_safe_region(self):
        ...
        orig_defreg = self.identity.get_default_region
        reg = utils.random_name()
        self.identity.get_default_region = Mock(return_value=reg)
        ret = pyrax._safe_region(identity=self.identity)
        self.assertEqual(reg, ret)
        self.identity.get_default_region = orig_defreg
    ...
    def test_connect_to_services(self):
        ...
        pyrax.connect_to_cloud_loadbalancers.assert_called_once_with(
                self.identity, region=None)
        pyrax.connect_to_cloud_databases.assert_called_once_with(self.identity,
                region=None)
    ...
    def test_connect_to_cloudservers(self):
        ...
        pyrax.connect_to_cloudservers = self.orig_connect_to_cloudservers
        pyrax.cloudservers = pyrax.connect_to_cloudservers(self.identity)
    ...
    def test_connect_to_cloud_databases(self):
        ...
        pyrax.connect_to_cloud_databases = self.orig_connect_to_cloud_databases
        pyrax.cloud_databases = pyrax.connect_to_cloud_databases(self.identity)
